package corejava.concurrency.adderSubtractorWithLocks;
...
public class Client {
    public static void main(String[] args) {
       Value value = new Value(); // Create a Value instance to hold the number
        Lock lock = new ReentrantLock(); // Create a lock for synchronization
       ExecutorService executor = Executors.newFixedThreadPool(2);

        Future<Void> adderFuture = executor.submit(new Adder(value, lock));
        Future<Void> subtractorFuture = executor.submit(new Subtractor(value, lock));
